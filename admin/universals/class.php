...
$_SESSION['address']= "Address is required !";
	...
	if(empty($var['birthdate'])){
  $phone= "No Phone !";

   } 
   else $birthdate= $var['birthdate'];
...
//print_r($var);
	...
	public function login($log){
		...
		if(preg_match('/^[a-zA-Z0-9]{5,}$/', $email)) { // for english chars + numbers only
    // valid username, alphanumeric & longer than or equals 5 chars
		...
		$row=$run->fetch_assoc();
		 ...
		 $up="update users set status='online' where username='$email' ";
		...
		$_SESSION['we']="Invalid Username or Password";
	
		}
		else $_SESSION['we']="Username can only contain letter and numbers, and longer than 5 chars!";

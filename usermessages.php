...
<div class="container  w-75 shadow border my-5">
	<div class="row top mt-5">
		...
		<span class="border shadow text-dark h6 py-2 bg-light d-inline-block mr-auto ml-5 px-4 my-4">Logged in: <?php $name=$_SESSION['name']; echo $name;?></span>

	
	<a href="logout.php" class="border shadow h6 text-dark nav-link bg-primary rounded d-inline-block ml-auto mr-5 px-4 my-4">Logout</a>
		
		
		
	</div>

	<?php $id=$_GET['id']; $user="select username from users where username='$id' "; $user=$db->select($user); $c_user=$user->fetch_assoc();
	                      $sts="select * from shoutmessages where USER_NAME='$id' "; $rows=$db->select($sts); 
	?>
	
	

<div class="row text-center  bg-light mb-5 py-2 font-weight-bold "><h3 class="text-dark m-auto">All Messages from: <?php echo $c_user['username'];?>
</h3></div>
...
<div class="col-sm-12 pt-1">
	...
	<div class="clearfix my-4"></div>
		  ...
		  <a href="messages.php" class="bg-light border border-dark text-dark text-center py-1 px-5 mt-5 w-50">Back</a>
